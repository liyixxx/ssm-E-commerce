	...
	public EgoResult create(MyOrderParam param, HttpServletRequest request) {
		...
		try {
			...
			if(index > 0){
				...
				if(orderParamList.size()==8){
					orderParamList.remove(7);
				}
				orderParamList.add(0,orderParam);
				// 将订单的信息存入到redis
				String orderListKey = orderKey + user.get("username")+":"+orderParam.getOrderShipping().getOrderId();
				jedisDaoImpl.set(orderKey, JsonUtils.objectToJson(orderParamList));
				// 购物车信息跟新
